...
const axiosClientUploadFile = axios.create({
  baseURL: `${API_URL}`,
  headers: {
    'Content-Type': 'multipart/form-data'
  },
});
axiosClientUploadFile.interceptors.request.use(
  async config => {
    const token = await AsyncStorage.getItem('token');
    ////console.log(token)
    if (token) {
...
axiosClientUploadFile.interceptors.response.use(
  response => {
    ...
    if (getToken() && error?.response?.status === 401) {
      ...
      if (!isRefreshing) {
        ...
        try {
          ...
          isRefreshing = false;
          return axiosClientUploadFile.request(error.config);
        } catch (error) {
          isRefreshing = false;
...
export default axiosClientUploadFile;